...
# --- Implement a two-layer network on toy data ---
...
toy_X, toy_y, params = get_toy_data(device="mps")
print('toy_X:')
print(toy_X)
print('toy_y:')
print(toy_y)
...
# --- Training & Prediction ---
from two_layer_net import nn_train, nn_predict, nn_forward_backward
...
from two_layer_net import TwoLayerNet, nn_get_search_params, find_best_net
...
val_acc = 100.0 * (y_val_pred == data_dict['y_val']).to(torch.float32).mean().item()
print('Validation accuracy: %.2f%%' % val_acc)


# The validation accuracy is lower than 10%, to debug we can plot the loss and accuracies, we can also visualize weights.
...
# --- Tune hyperparameter: hidden_size, reg, learning_rate ---

# The model is underfitting, try to increase hidden_size to increase ite capacity.
...
# Regularization: if it is too high, the model hardly learn anything
hs = 128
lr = 1.0
...
val_acc = 100 * (y_val_preds == data_dict['y_val']).to(torch.float32).mean().item()
...
test_acc = 100 * (y_test_preds == data_dict['y_test']).to(torch.float32).mean().item()
