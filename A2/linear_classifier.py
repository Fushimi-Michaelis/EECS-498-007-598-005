...
def svm_loss_vectorized(
    ...
    scores_correct = scores[torch.arange(num_train), y].reshape(num_train, 1)
    margin = scores - scores_correct + 1 # delta is 1
    # Set margin of correct label to 0
    margin[torch.arange(num_train), y] = 0
    # Calculate loss
    loss_matrix = torch.clamp(margin, min=0)
    ...
    #############################################################################
    # Replace "pass" statement with your code
    mask = torch.zeros_like(loss_matrix)
    mask[loss_matrix > 0] = 1
    dW = torch.zeros_like(W)
    num_positive_maigins = mask.sum(dim=1)
    mask[torch.arange(num_train), y] = -num_positive_maigins
...
def test_one_param_set(
    ...
    # Train cls with X_train
    cls.train(X_train, y_train, lr, reg, num_iters, verbose=True)

    # Prediction
    y_train_pred = cls.predict(X_train)
    y_val_pred = cls.predict(X_val)

    # Check accuracy on train and val dataset
    train_acc = 100.0 * (y_train == y_train_pred).to(torch.float32).mean().item()
    val_acc = 100.0 * (y_val == y_val_pred).to(torch.float32).mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        loss -= scores_stable[i][y[i]]
        sum_j = 0
        for j in range(num_label):
            sum_j += torch.exp(scores_stable[i][j])
    ...
    # Gradient
    for i in range(num_train):
        scores_stable_exp = torch.exp(scores_stable[i])
        prob_i = scores_stable_exp / scores_stable_exp.sum()
        for j in range(num_label):
            dscore = prob_i[j]
            if j == y[i]:
                dscore -= 1
...
def softmax_loss_vectorized(
    ...
    max_scores, _ = torch.max(scores, dim=1, keepdim=True)
    scores_stable = scores - max_scores
    loss = -torch.sum(scores_stable[torch.arange(num_train), y]) + torch.sum(torch.log(torch.exp(scores_stable).sum(dim=1)))
    loss /= num_train
    loss += reg * torch.sum(W * W)
    scores_stable_exp = torch.exp(scores_stable)
    prob = scores_stable_exp / scores_stable_exp.sum(dim=1, keepdim=True)
    descrease = torch.zeros_like(scores)
    descrease[torch.arange(num_train), y] = -1
    prob += descrease
