...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden_pre_activation = X.matmul(W1) + b1

    # Activation function: ReLU
    # Create a tensor with only one "0.0" and utilize broadcasting 
    # Save memory compared with torch.zeros_like(hidden_pre_activation)
    hidden = torch.maximum(hidden_pre_activation, torch.tensor(0.0, dtype=X.dtype, device=X.device))
...
def nn_forward_backward(
    ...
    loss = -prob[torch.arange(N), y].log().sum() / N
    ...
    # Second layer: FC layer
    grads["W2"] = h1.T.matmul(grad_scores)
    grads["b2"] = grad_scores.sum(dim=0)
    grad_h1 = grad_scores.matmul(W2.T)

    # Activation layer: ReLU
    grad_h1[h1 <= 0] = 0

    # First layer: FC layer
    grads["W1"] = X.T.matmul(grad_h1)
    grads["b1"] = grad_h1.sum(dim=0)
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        for p_name in params:  
            params[p_name] -= learning_rate * grads[p_name]
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def find_best_net(
    ...
    param_combinations = len(learning_rates) * len(hidden_sizes) * len(regularization_strengths) * len(learning_rate_decays)
    # Consider using itertools.product to optimize this huge loop
    i = 0
    for hs in hidden_sizes:
        for lr in learning_rates:
            for reg in regularization_strengths:
                for lrd in learning_rate_decays:
                    print(f"[{i+1}/{param_combinations}] Testing: lr={lr:.1e}, reg={reg:.1e}, hidden_size={hs}, lr_decay={lrd}")
                    i += 1
                    ...
                    stats = net.train(data_dict['X_train'], data_dict['y_train'],
                                     ...
                                     num_iters=3000, batch_size=1000, verbose=True)
                    val_last = stats['val_acc_history'][-1]
                    print(f'Validation accuracy on the last epoch is: {val_last:.2f}')
                    if val_last > best_val_acc:
                        best_val_acc = val_last
                        best_net = net
                        best_stat = stats
    #############################################################################
    #                               END OF YOUR CODE                            #
