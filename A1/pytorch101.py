...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    x[0:2, 1] = 1
    x[0:2, 2:6] = 2
    x[2:4, 0:1] = x[2:4, 2:3] = 3
    x[2:4, 1:2] = x[2:4, 3:4] = 4
    x[2:4, 4:6] = 5
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################s
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.zeros(3, 8)
    y[0:3, 0:4] = x[0:12].reshape(3, 4)
    y[0:3, 4:8] = x[12:24].reshape(3, 4)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def mm_on_gpu(x: Tensor, w: Tensor) -> Tensor:
    ...
    w_mps = w.to(mps_device)
    
    y_mps = x_mps.mm(w_mps)
